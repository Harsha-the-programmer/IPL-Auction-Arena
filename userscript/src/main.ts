...
(() => {
  ...
  // Configuration
  const API_BASE = (typeof GM_info !== 'undefined' && GM_info.scriptHandler === 'Tampermonkey')
  ...
  function showToast(type: 'success' | 'error' | 'info', title: string, text: string) {
  ...
  function setButtonState(btn: HTMLButtonElement, state: 'idle' | 'exporting' | 'success' | 'error', text: string) {
  ...
  function transformRoomState(state: any): any {
    ...
    for (const [teamId, teamData] of Object.entries(state.teams || {})) {
      ...
      const teamInfo = IPL_TEAMS.find(ti => ti.id === teamId) || { id: teamId, name: teamId, shortName: teamId, color: '#64748b' }
  ...
  function mapRole(role: string): 'BATTER' | 'BOWLER' | 'ALL_ROUNDER' | 'WICKET_KEEPER' {
      ...
      GM_xmlhttpRequest({
        ...
        onload: (resp) => {
          const response = new Response(resp.responseText, {
            ...
            headers: new Headers(resp.responseHeaders as Record<string, string>),
          })
          resolve(response)
        },

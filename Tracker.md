...
| **4.2** | Refresh-safe room resume           | ✅ Done        | 2026-08-11 | 2026-08-11 | Route-level resume skips name form after refresh |
...
- Completed: Fixed room refresh flow so the socket provider receives the route room ID, resumes by persisted client ID without recreating kicked participants, and only shows the name form when resume is not possible.
...
- Next: Manually verify refresh/rejoin behavior with a running server and deployed socket environment

...
export default function RoomPage({ params }: { params: { roomId: string } }) {
  ...
  const handleNameSubmit = () => {
    if (displayName.trim().length >= 2) {
      // Join the room with the entered name
  ...
  const pendingTeams =
    room?.teams.filter((t) => t.claimStatus === "PENDING") || [];
  const approvedTeams =
    room?.teams.filter((t) => t.claimStatus === "APPROVED") || [];
  const allLocked =
          ...
          <div className="mb-6 p-4 bg-amber-500/10 border border-amber-500/30 rounded-xl">
            ...
            <div className="flex flex-wrap gap-3">
              {pendingTeams.map((team) => (
                <div
                  key={team.teamId}
                  className="flex items-center gap-3 px-4 py-2 bg-neutral-800/50 rounded-lg"
                >
                  <div
                  ...
                  <span className="font-medium">
                    {team.requestedByName} wants {team.name}
                  </span>
                  <button
                    onClick={() =>
                      socket?.emit("team:approve", {
                        teamId: team.teamId,
                        socketId: team.requestedBySocketId!,
                      })
                    }
            ...
            {IPL_TEAMS.map((teamConfig) => {
              ...
              return (
                ...
                >
                  {/* Team Badge */}
                  <div
                    className="w-14 h-14 mx-auto mb-3 rounded-full flex items-center justify-center text-white font-bold text-2xl"
                    style={{ backgroundColor: team.color }}
                  >
                    {team.shortName}
                  </div>
        ...
        <div className="mb-8">
          ...
          <div className="grid gap-2">
            {room?.participants.map((p) => (
              <div
                key={p.socketId}
                className="flex items-center gap-3 p-3 bg-neutral-900/50 rounded-lg"
              >
                ...
                <div className="flex-1">
                  <p className="font-medium flex items-center gap-2">
                    {p.displayName}
                    {p.isHost && (
                      <span className="badge-warning text-xs">Host</span>
                    )}
                  ...
                  {p.teamId && (
                    <p className="text-xs text-neutral-500">
                      Team:{" "}
                      <span className="font-medium text-white">{p.teamId}</span>
                    </p>
                  )}
                </div>
                <div
                  className={cn(
                    ...
                    p.isOnline ? "bg-green-500" : "bg-neutral-600",
                  )}
                />
              </div>
            ))}
        ...
        {isHost && approvedTeams.length > 0 && (
          <div className="border-t border-neutral-800 pt-6">
            ...
            >
              {allLocked
                ? "🚀 Start Match"
                : `Waiting for ${approvedTeams.filter((t) => !t.isLocked).length} team(s) to lock lineup`}
            </button>
            {!allLocked && (

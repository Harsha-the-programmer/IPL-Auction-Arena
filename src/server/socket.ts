...
function mapTeamToState(team: any): TeamState {
  return {
    ...
    lineup: team.lineup
      ? team.lineup.slots.sort((a: any, b: any) => a.position - b.position).map(mapLineupSlotToState)
      : null,
...
io.on('connection', (socket: Socket<ClientToServerEvents, ServerToClientEvents, {}, SocketData>) => {
  ...
  socket.on('room:join', async ({ roomId, displayName }) => {
    try {
      ...
      // Notify others
      if (isNewParticipant) {
        socket.to(`room:${roomId}`).emit('user:joined', mapParticipantToState(participant))
      }

      console.log(`[Socket] ${displayName} joined room ${roomId} (host: ${participant.isHost})`)
    } catch (error) {
  ...
  socket.on('team:reject', async ({ teamId, socketId: targetSocketId }) => {
    ...
    try {
      ...
      io.to(`room:${roomId}`).emit('team:rejected', { teamId, userId: targetSocketId })
      io.to(targetSocketId).emit('room:state', room)
    } catch (error) {
    ...
    try {
      ...
      if (team.lineup) {
        team.lineup = lineupSlots.map(s => ({
          ...
          playerId: s.playerId,
          player: s.playerId ? team.players.find(p => p.id === s.playerId) || null : null,
          isLocked: false,
        }))
      }
      setRoomCache(roomId, room)
  ...
  socket.on('match:start', async () => {
    ...
    try {
      ...
      const approvedTeams = room.teams.filter(t => t.claimStatus === 'APPROVED')
      const allLocked = approvedTeams.every(t => t.isLocked && t.lineup?.length === 11)
      if (!allLocked) return
  ...
  async function getAIRanking(roomId: string, picks: PickState[], positions: number[]) {
    ...
    // Calculate points
    const activeTeams = picks.map(p => p.teamId).filter((v, i, a) => a.indexOf(v) === i).length
    ...
    // Emit ranking
    io.to(`room:${roomId}`).emit('round:ranking', { ranking: ranked, points })
  ...
  async function completeRound(roomId: string) {
    ...
    io.to(`room:${roomId}`).emit('round:complete', { leaderboard: teamScores })
    ...
    await prisma.room.update({
      where: { id: roomId },
      data: { currentRound: room.currentRound, currentPosition: room.currentPosition },
    })
  ...
  async function endMatch(roomId: string, finalStandings: TeamScore[]) {
    ...
    io.to(`room:${roomId}`).emit('match:complete', { finalStandings, winner })
  }
